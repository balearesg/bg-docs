...
Abre un editor de texto y crea un nuevo archivo llamado `prettier.json`.
Copia y pega el siguiente código en el archivo `prettier.json`:
...
Guarda el archivo prettier.json en la raíz de tu proyecto
...
1. Abre la configuración de tu editor. Esto generalmente se hace a través del menú **"Preferences"** (Preferencias) o "Settings" (Configuración) en el editor.

2. Busca la configuración de Prettier. Puede aparecer como **"Prettier"** o **"Format"** en la configuración del editor. Selecciona la configuración correspondiente para Prettier.

3. Dentro de la configuración de Prettier, busca la opción **"Format On Save"** (Formatear al Guardar) y actívala. Esta opción indica al editor que ejecute automáticamente Prettier cada vez que guardes un archivo.

4. Guarda los cambios en la configuración y cierra la ventana de configuración.

5. A partir de ahora, cuando guardes un archivo en tu editor de código, Prettier se ejecutará automáticamente y formateará el código según las reglas y configuraciones establecidas en tu archivo prettier.json.
